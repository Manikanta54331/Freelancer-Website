...
<section class='hero'>
    <table align='center' border='1' id='tab'>
        ...
        try {
            ...
            String sql = "SELECT * FROM addprojects WHERE fid = fid";
            stmt = conn.prepareStatement(sql);
            rs = stmt.executeQuery();
            int i = 1;
            while (rs.next()) {
                out.println("<tr><td>" + i + "</td><td>" + rs.getString("projectName") + "</td><td>" + rs.getString("projectDescription") + "</td><td>" + rs.getString("projectBudget") + "</td><td>" + rs.getString("projectDuration") + "</td><td>" + rs.getString("projectDeadline") + "</td><td>" + rs.getString("fname") + "</td><td>" + rs.getString("cname") + "</td><td><form name='status' action='Update.jsp' method='post'><select name='status1'><option value='Assigned'>Assigned</option><option value='Ongoing'>Ongoing</option><option value='Completed'>Completed</option></select></td><td><input type='submit' name='submit' value='update'/></form></td></tr>");
                i++;
            }
